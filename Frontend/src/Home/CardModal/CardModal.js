...
{ 
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // Get item details
        const itemRes = await axios.get(`http://localhost:4000/login/item/${itemId}`);
        ...
        // Get poster details
        const userRes = await axios.get(`http://localhost:4000/login/user/${sid}`);
         setPoster(userRes.data);
  
        // Check if the logged-in user is the owner of the item
        const resultRes = await axios.get('http://localhost:4000/login', {
          headers: {
    ...
    fetchData();
      ...
      {   
        ...
        axios.post("http://localhost:4000/login/message",[{ "sid":sid, "message": message, "itemName": name} ], {
          headers: {
            withCredentials: true,
            cook: cookies.get("access-token")
        }})
